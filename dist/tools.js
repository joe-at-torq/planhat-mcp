...
const idProp = {
    id: {
        type: "string",
        description: "Planhat _id, or use 'ext-{externalId}' / 'src-{sourceId}' prefixes to look up by external IDs",
    },
};
function registerCRUD(def) {
    const { path, singular, label, createRequired = [], createProperties, listExtraParams = {}, bulkUpsert = true, companyFilter = true, } = def;
    ...
    }, (client, args) => client.getById(path, args.id));
    // Create
    add({
    ...
    add({
        ...
        const { id, ...rest } = args;
        return client.update(path, id, rest);
    });
    // Delete
    add({
    ...
    }, (client, args) => client.remove(path, args.id));
    // Bulk upsert
    if (bulkUpsert) {
        add({
        ...
        }, (client, args) => client.bulkUpsert(path, args.items));
    }
}
...
add({
    ...
    description: "Get a lightweight list of companies with only _id, name, externalId, and sourceId. Intended for ID lookups and bulk mapping — not for name searches. Use find_company_by_name to search by name.",
    inputSchema: {
        type: "object",
        properties: {
...
add({
    ...
    description: "Search for companies by name. Fetches the lean company list (only _id, name, externalId, sourceId) to find matches efficiently, then retrieves full details only for the current page of results. Always prefer this over list_companies when looking up a company by name. Use 'page' and 'page_size' to paginate through large result sets without flooding context.",
    inputSchema: {
        type: "object",
        properties: {
            name: {
    ...
    const lean = (await client.list("leancompanies", {}));
    ...
    const results = await Promise.all(pageMatches.map((c) => client.getById("companies", c._id)));
    return { total_matches: totalMatches, page, total_pages: totalPages, results };
});
registerCRUD({
    path: "endusers",
...
}, (client, args) => client.getById("customfields", args.id));
add({
    ...
    inputSchema: {
        ...
        properties: {
            ...
            parent: {
                type: "string",
                description: "Parent model this field belongs to: Company, EndUser, Asset, Project, Conversation, etc.",
            },
            type: {
                type: "string",
                enum: [
...
add({
    ...
    const { id, ...rest } = args;
    return client.update("customfields", id, rest);
});
add({
...
}, (client, args) => client.remove("customfields", args.id));
...
}, (client, args) => client.getById("users", args.id));
// ---- Metrics / Dimension Data ----
add({
    name: "push_metrics",
    description: "Push dimension data (metrics/analytics) into Planhat. Requires PLANHAT_TENANT_UUID to be configured.",
    inputSchema: {
        type: "object",
        properties: {
            items: {
...
}, (client, args) => client.pushMetrics(args.items));
add({
    name: "get_metrics",
